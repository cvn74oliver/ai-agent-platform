...
import type { RuntimeConfidenceActionSummary, RuntimeMode } from '@/lib/runtime/types'
...
export async function GET(req: Request) {
  try {
    ...
    if (modeError) {
      console.error('[runtime/eligibility] mode lookup error:', modeError)
      return NextResponse.json(
    ...
    if (error) {
      console.error('[runtime/eligibility] confidence lookup error:', error)
      return NextResponse.json(
        { ok: false, error: 'Failed to load eligibility data.' },
        { status: 500 }
      )
    }
    ...
    console.error('[runtime/eligibility] Unexpected error:', err)
    return NextResponse.json(
      { ok: false, error: 'Unexpected error in /api/runtime/eligibility.' },
      { status: 500 }
    )
  }
}
