...
- Current gate: APPROVED SAME-FLOW EXECUTION / candidate-version runtime verification. The authoritative packet is `docs/00_control_plane/runtime/ACE-048_ALL_PARENT_CLEANUP_TAXONOMY_CONSOLIDATION_PM_BRIEF.md`. Preserve every lineage and the April publication while verifying the one unpublished candidate through the development-only override. Publication, local-main promotion, push, deployment, and branch/worktree deletion remain unauthorized.
...
- Exact next step: rerun the static/build checkpoint, then use the development-only artifact override for post-settle Playwright on the canonical Cleanup Groups route. Human Review checkpoint one is not requested until browser proof passes. Publication, main merge/promotion, push, deployment, and lineage retirement remain blocked.
