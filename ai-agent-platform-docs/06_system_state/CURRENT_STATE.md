...
- Exact next step: wire candidate-version projection reads into the canonical review route, preserve fixed membership across observation windows, apply the bounded Pressure Trend correction, then run static/build and post-settle Playwright correction proof. Publication, Smart Sync, Gmail reindex, deployment, push, local-main promotion, and lineage retirement remain unauthorized.
- Status: `ACTIVE — ACE-048 Stage D Runtime Integration + Correction Proof`.
