...
- Post-merge publication recovery is HUMAN REVIEW `REJECT` / `RETURN_TO_PM` as of 2026-08-31. The guarded publication of August artifact `full-mailbox-20260825031402535` remains the active runtime fact, but it is not an Accepted Fix: yearly Pressure Trend visibly has empty May and June 2026 buckets before July resumes; ordinary child units can be too small to justify separate top-level choices; and non-`All Indexed` Sender Distribution / Time Context windows fail on additional chooser-generated review-unit identities even though the previously verified `family:editorial_newsletter` `1W` path passed.
- The prior verifier packet in `docs/00_control_plane/runtime/ACE-048_POST_MERGE_PUBLICATION_RECOVERY_PM_BRIEF.md` is retained as bounded evidence, not closeout authority. Active PM discovery is split into three work packets: indexed-source continuity, framework-first small-group presentation policy, and representative all-child narrowed-window projection/runtime diagnosis. No Accepted Fix, Recovery Contract, main merge, GitHub push, deployment, Smart Sync, reindex, rebuild, rollback, or further publication is authorized.
...
- Authoritative pending plan: `docs/00_control_plane/runtime/ACE-048_HUMAN_REVIEW_RETURN_CORRECTION_PM_BRIEF.md`. It proposes manifest-authoritative projection identity, gap-aware bounded index bridging, and a framework-first chooser policy that keeps exact membership while moving ordinary `<5` and `<1%` units/overflow under More specific and risk/action exceptions under Special handling. Status: `Awaiting Decision`; source edits and live repair remain unauthorized.
...
- Exact next step: obtain Oliver's separate decision on the reviewed Management discovery direction, then either propagate an approved provider-neutral Management PM Brief or revise the recommendation. Deployment, artifact publication, rebuild, sync, reindex, Supabase mutation, and recovery-branch deletion remain separately gated.
