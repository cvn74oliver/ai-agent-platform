...
- Decision Mode optional-evidence availability is **HUMAN-ACCEPTED** as of 2026-08-30 on preservation branch `codex/ace-048-decision-mode-optional-evidence`, derived from accepted local-main baseline `7526afe49235265f0b257fbb3dd4a389c7ea129c`. Oliver accepted the direct authenticated root proof on the exact canonical port-`3001` Editorial/content route: full `76` and narrowed `1M` `25` paths reached ready state; subject/date evidence remained usable when optional detail degraded; settle produced zero repeat snippet requests; explicit retry issued exactly one request; close/return preserved selection and nonzero rows; and no `412` or `409` churn remained. Recovery Contract: `CHANGELOG.md` -> `August 30, 2026 — ACE-048 Decision Mode Optional-Evidence Availability Accepted`.
- The accepted consolidation remains local only. `origin/main`, GitHub, deployment, artifact publication, Gmail/index state, Supabase data, and every preserved recovery branch remain unchanged. Remote push/deployment and any branch deletion require separate explicit decisions.
- Local `main` now contains the intentionally consolidated ACE-048 source through `7866368c97a6ca8d241a9541f6f83570df2017f4` and, after the 2026-08-30 retirement checkpoint, is `22` commits ahead of the untouched `origin/main` baseline `64632b3faa0736cdf15534b4465cdef8a404a4e8`. The governed pre-consolidation recovery branch remains `codex/ace-048-main-preconsolidation` at `22148cef9fc15e82730f19ef2f35eb3829763931`; all named archive and integration branches remain preserved.
- Worktree retirement completed on 2026-08-30: the seven obsolete ACE-048 checkouts were removed after exact branch/HEAD reattestation. Local `main` is the only remaining checkout. Every named recovery branch remains intact; no branch was deleted. Dirty state discarded from `33ad`, `7865`, and `a985` was limited to explicitly excluded generated Playwright output and saved-auth residue, not product/control-plane source.
...
- Exact next step: integrate `codex/ace-048-decision-mode-optional-evidence` into local `main`, then repeat the accepted canonical Decision Mode proof on the normal port-`3000` runtime before Management work begins. Non-force remote push/deployment remains a separate decision. No push, deployment, publication, rebuild, sync, reindex, Supabase mutation, or recovery-branch deletion is authorized by this acceptance.
- Status: `LOCAL MAIN ANALYSIS HUMAN-ACCEPTED / DECISION MODE FIX HUMAN-ACCEPTED ON PRESERVATION BRANCH / LOCAL-MAIN INTEGRATION OPEN / NO PUSH`.
- Checkpoint Status: continuity checkpoint created for local-main integration and normal port-`3000` re-verification; accepted-fix truth and Recovery Contract are propagated on the preservation branch.
...
- Worktree/branch truth: only main is presently checked out as a named product worktree; detached Codex preservation worktrees remain. The cleanup checkout itself is gone, but local/origin `cleanup-taxonomy-rebuild` at `c690dff` is preserved. It diverges from main at merge base `13070fa`; it has not been fully promoted or merged. The current live tree comparison is `84` cleanup-changed paths, `45` now byte-identical on main, and `39` disposition paths (`8` cleanup-unopposed, `31` both-changed-differently). Sender-distribution work already ported to main remains present, while the cleanup branch is still mandatory feature-authority evidence for the remaining semantic-union integration.
