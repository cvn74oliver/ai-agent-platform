...
- Post-merge publication recovery is VERIFIER-PASSED / AWAITING HUMAN DECISION as of 2026-08-31. The guarded compare-and-set publication moved canonical `all_indexed` from April artifact `full-mailbox-20260415024237593` to validated August artifact `full-mailbox-20260825031402535`; publication is now `published / fresh`, the build lock is null, the job phase is `published`, and the immutable candidate retains `5,024` senders, `244,628` messages, `7` parents, `60/60` validated review-unit manifests, and coverage through `2026-08-15`.
- The bounded recovery authority and verifier packet are recorded in `docs/00_control_plane/runtime/ACE-048_POST_MERGE_PUBLICATION_RECOVERY_PM_BRIEF.md`. Candidate-preview and normal port-`3000` proof both pass after settle across Mailbox Intelligence, Cleanup Groups, the canonical `Newsletters and editorial updates` child, Sender Distribution `1W`, and Time Context `1W`. Mailbox Intelligence exposes read-only indexed coverage rather than a competing dropdown, and Pressure Trend reaches Q3 2026. No API failure, `409` guard churn, duplicate-key warning, console error, Gmail access, Smart Sync, reindex, rebuild, migration, deployment, or wholesale cleanup-branch integration occurred.
