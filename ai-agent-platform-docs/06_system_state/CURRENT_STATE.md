...
- Current gate: `Status: Awaiting Decision` — Cleanup Groups Human Review checkpoint one. The authoritative packet is `docs/00_control_plane/runtime/ACE-048_ALL_PARENT_CLEANUP_TAXONOMY_CONSOLIDATION_PM_BRIEF.md`. Preserve every lineage and the April publication while Oliver reviews the verified unpublished candidate. Publication, local-main promotion, push, deployment, and branch/worktree deletion remain unauthorized.
...
- Exact next step: Oliver performs Human Review checkpoint one for Cleanup Groups parent/child names, intuitiveness, and navigation on the candidate override. If accepted, integrate and verify the preserved Sender Distribution and Pressure Trend lineage; do not publish or promote yet.
