...
- Phase 4 Slice 4A Stage C1 target lock and implementation authority — 2026-09-03: read-only discovery reattested live project `cjpjekhlvzwjwtszqpmy`, exact four-function identities/security, zero rows in both ledger tables, no non-internal triggers/publication membership, and a viable single-transaction rollback path. Oliver issued exact `ACCEPT PHASE 4 SLICE 4A STAGE C1 ROLLBACK-ONLY RPC PROOF`. The execution-ready PM Brief is `docs/00_control_plane/runtime/ACE-048_FRAMEWORK_FIRST_DECISION_WORKSPACE_PHASE4_SLICE4A_STAGEC1_ROLLBACK_ONLY_RPC_PROOF_PM_BRIEF.md`. Authority is limited to the governed backup, fixed synthetic sandbox identities, at most `15` function calls inside one explicit `BEGIN` / `SET LOCAL ROLE service_role` / `ROLLBACK` transaction, exact success/replay/partial/stale/cross-tenant assertions, and independent post-rollback zero-persistence proof. Source activation, HTTP routes, providers, customer data, persistent test rows, migrations, merge, and deployment remain blocked.
