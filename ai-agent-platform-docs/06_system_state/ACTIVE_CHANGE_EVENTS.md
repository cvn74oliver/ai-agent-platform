...
- Next governing step: integrate the accepted preservation branch into local `main`, then repeat the canonical proof on the normal port-`3000` runtime before Management work. Non-force remote push/deployment remains separately gated. Publication, rebuild, Smart Sync, Gmail reindex/backfill, Supabase mutation, deployment, push, and recovery-branch deletion remain unauthorized.
- Checkpoint Status: continuity checkpoint created for local-main integration and normal port-`3000` re-verification. Accepted-fix truth and Recovery Contract are propagated; no external closeout authority is implied.
