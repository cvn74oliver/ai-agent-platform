...
- Stage D load/lifecycle contract: one bounded projection family per changed review-unit/window key, single-flight ownership, fixed membership retained through rows/pagination/Decision Mode/distribution, zero recurring heavy requests after settle, one-attempt-per-key Pressure Trend acquisition, and no navigation-triggered build/sync/reindex/publication behavior.
- Status: `ACTIVE — ACE-048 Stage D Runtime Integration + Correction Proof`.
- Checkpoint Status: none; Stage D approval and scope are synchronized before implementation. This is not an Accepted Fix or publication authority. Main promotion, push, deployment, Smart Sync, Gmail reindex, publication, and lineage retirement remain unauthorized.
