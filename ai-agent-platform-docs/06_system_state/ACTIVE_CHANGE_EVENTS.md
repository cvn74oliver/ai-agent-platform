...
Status: PM PLAN / AWAITING OLIVER DECISION
Problem class: mixed but decomposed — source/index continuity, runtime projection identity, and cleanup-choice presentation policy
...
Checkpoint Status: continuity checkpoint created. Human Review rejection, root-cause diagnosis, and the three bounded correction contracts are propagated. Status is `Awaiting Decision`; no implementation or live repair is authorized until Oliver records `ACCEPT`.
