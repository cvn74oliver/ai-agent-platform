...
## Shared Rail Alignment (CRITICAL)
This chart is NOT a standalone surface.

It exists inside the **Shared Analysis Rail** alongside:
- Time Context (activity chart)

Rules:
- Only ONE full-width chart is visible at a time (tab-based)
- This chart must NOT replace Time Context
- Both charts share:
  - timeframe controls
  - workflow scope
  - underlying data truth
...
## Role in Decision System

This chart is not just analytical — it is a **decision prioritization engine**.

It must:
- rank senders using artifact-backed truth
- drive workflow subset behavior
- determine which sender is handled first in Decision Mode

It must NOT:
- introduce a separate queue model
- diverge from the authoritative execution order
...
- Use a tab system inside a shared full-width container
...
- Integrate with shared workflow subset contract
- Ensure chart drives workflow + Decision Mode behavior
...
### Ranking authority
- Ranking must come from artifact-backed ordering
- Client must NOT re-sort independently
- Chart must mirror the same order used by workflow list and Decision Mode
...
- update shared workflow subset contract:
  - `kind = 'focused_sender'`
...
- Decision Mode opens that sender first

Important:
- This does NOT create a new queue
- It reuses the same authoritative execution order
...
This chart must follow shared scope rules:

- If `workflow_scope` is READY:
  - it becomes the authoritativeScope
  - chart updates to that scope
  - workflow list + Decision Mode update together

- If `workflow_scope` is NOT READY:
  - chart remains on current authoritativeScope
  - no workflow change occurs
  - UI shows comparison-only state

Invariant:
- chart must never switch to an empty subset when a ready subset exists
...
- Chart exists inside tabbed rail, not as standalone section

---

## No-Rehydrate Guard (CRITICAL)

Chart interactions must NOT trigger:
...
All updates must be handled using existing in-memory or artifact-backed data.
...
- Decision Mode first sender matches chart rank
- clicking sender updates workflow and Decision Mode consistently
- no new rehydrate events occur
